...
debuglog = require("debug")("redis-backup::utils::mailer")
assert = require "assert"
...
exports.deliverServerException = (error) ->
  ...
  request
    ...
    json :
      "From" : DEFAULT_FROM
      "To" : TO
      "Subject" : "redis-backup::#{SERVER_NAME}::uncaughtException #{(new Date).toLocaleString()}, #{error}"
...
exports.sendErrors = (errors) ->
  ...
  request
    ...
    json :
      "From" : DEFAULT_FROM
      "To" : TO
      "Subject" : "redis-backup::#{SERVER_NAME}::errors #{(new Date).toLocaleString()}"
